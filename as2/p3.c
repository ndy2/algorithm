...
void print_treeInfo(treeInfo ti){
...
bool isLeaf(node * n){
    return (n->val != NILVAL) && n->left->val ==NILVAL && n->right->val ==NILVAL;
}


void dfs_findLeaves(node * cur){
    if(isLeaf(cur)){
        temp[idx] = cur->val;
        idx++;
    }else if(cur->val != NILVAL){
        dfs_findLeaves(cur->left);
        dfs_findLeaves(cur->right);
    }
}

void setTreeInfo(tree t, treeInfo* ti, char * treeName){
    ti->treeName = treeName;
    ti->rootId = t.root->val;
...
int main(){
    ...
    // deleteArr(&t,delArr,5,true);

    treeInfo ti;
    setTreeInfo(t, &ti, "Tree Name");
    print_treeInfo(ti);
}
